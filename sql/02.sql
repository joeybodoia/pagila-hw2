...
--select rating, count(rating) from film group by rating order by count DESC limit 2;


--select rating from film group by rating order by count(rating) DESC limit 2;

select film_id, title from film where rating in (select rating from film group by rating order by count(rating) DESC limit 2) order by title;
